...
本專案支援透過 Unity Package Manager (UPM) 的 Git URL 進行安裝。

1. 在 Unity 中開啟 `Window > Package Manager`。
2. 點擊左上角的 `+` 號按鈕，選擇 **Add package from git URL...**。
3. 貼上以下網址（包含 `?path=` 參數來指定套件資料夾）：
   ```text
   https://github.com/Wenrong274/spritebatchslicer.git?path=Packages/com.wenrong.spritebatchslicer
   ```
4. 點擊 **Add** 即可完成安裝。

## 使用方式

1. 安裝套件或在 Unity 開啟此專案。
